...
import footerImages from "../../../src/models/footerImages";
import { FileHandler } from "../../utils/fileHandler";
const fileHandler = new FileHandler();
export const createFooterPageContent = async (req: Request, res: Response) => {
  try {
    ...
    const newContent = new footerImages({
      images: mediaIds,
    });
...
export const getFooterPageContents = async (_: Request, res: Response) => {
  try {
    const contents = await footerImages.findOne().populate("images"); // Populate to get full Media document
    res.status(200).json(contents);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
export const updateFooterPageContent = async (req: Request, res: Response) => {
  try {
    const { existingImageIds } = req.body;

    // Fetch the current document from the database
    const existingContent = await footerImages.findOne();
    if (!existingContent) {
    ...
    const updatedContent = await footerImages.findOneAndUpdate(
      {},
      existingContent,
    );
